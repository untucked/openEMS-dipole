...
master_log_path = os.path.join(script_directory, 'dipole_sim', 'resonance_summary.txt')
if not os.path.exists(master_log_path):
    with open(master_log_path, 'w') as file_write:
        file_write.write('Length (mm), Resonant frequency (GHz)\n')
    print('Created master log file:', master_log_path)



def run_dipole_sim(dipole_length=150):
    Sim_Path = os.path.join(tempfile.gettempdir(), f'Dipole_L{dipole_length}mm')
    if not os.path.exists(Sim_Path):
        os.makedirs(Sim_Path)
    save_img_path = os.path.join(script_directory, 'dipole_sim', f'{dipole_length}mm')
    ...
    x1 = np.linspace(-dipole_length-10, -20, 17)   # 17 pts from –150 to –20 mm
    x3 = np.linspace(  20, dipole_length+10, 17)   # 17 pts from +20 to +150 mm
    ...
    with open(master_log_path, 'a') as file_write:
        file_write.write(f"{dipole_length} mm -> {f_res/1e6:.4f} MHz\n") 
