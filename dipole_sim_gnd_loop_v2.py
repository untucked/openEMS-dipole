...
master_log_path = os.path.join(data_dir, 'resonance_gnd.csv')

# Only create and write header if file doesn't exist
if not os.path.exists(master_log_path):
    with open(master_log_path, mode='w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Length (mm)', 'Fres (MHz)', 'D (dB)','Efficiency (%)',
                         'Gd (dBi)', 'Gr (dBi)', 'GND Distance (mm)', 'GND Cond (%)'])

    print('Created master log file:', master_log_path)

def run_dipole_sim(dipole_length=150, gnd_percent=100, Ftarget_MHz=891.5):
    ...
    f_target = Ftarget_MHz * 1e6  # target frequency (Hz)
    # ─── Feed settings ────────────────────────────────────────────────────────────
    ...
    if fast_sim:
        ...
        FDTD = openEMS(NrTS=40000, EndCriteria=1e-4) # FDTD = openEMS(NrTS=30000, EndCriteria=1e-4)
    ...
    if include_gnd:
        ...
        gnd_offset = radius + 1 # in mm;  lambda0_mm / 2 
        ...
        if AddConductingSheet:
            ...
            gnd.SetMaterialProperty(epsilon=1.0, kappa=int(gnd_conductivity))
        else :
            gnd = CSX.AddMetal('gnd_plane')

        # gnd = CSX.AddMetal('gnd_plane')
        gnd.AddBox(
            [-gnd_size/2, gnd_y, -10*radius],   # [X_start, Y, Z_start]
            [ gnd_size/2, gnd_y,  10*radius]    # [X_stop,  Y, Z_stop]
        )
    ...
    f = np.linspace(max(500e6,f0-fc),f0+fc,401)
    ...
    print('S11: {} dB'.format(s11_dB_val))
    print('S11: {}'.format(s11[idx])) 
    ...
    plot_sims.plot_Zin(f, Zin, title_val, save_img_path)
    Z_res = Zin[idx]
    # Z_match = np.conj(Z_res)
    Z_match = np.complex128(82.83259020907973+8.263688810128034j)
    s11 = (Zin - Z_match) / (Zin + Z_match)
    s11_dB = 20.0 * np.log10(np.abs(s11))
    ...
    title_val = 'Fmatch:{} MHz'.format(Ftarget_MHz)
    plot_sims.plot_S11(f, s11_dB, title_val, save_img_path,
             f_res, idx,
    ...
    E_norm = 20.0*np.log10(nf2ff_res.E_norm[0]/np.max(nf2ff_res.E_norm[0])) + D
    ...
    # Directivity in dB
    Gc = D + 10 * np.log10(efficiency / 100)
    Gr = 10 * np.log10(10**(Gc / 10) * mismatch_loss_factor)
    print(f"Efficiency = {efficiency:.2f}%")
    print(f"Gain = {Gc:.2f} dBi")
    print(f"Realized Gain = {Gr:.2f} dBi")

    with open(master_log_path, mode='a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([f"{dipole_length:.1f}", f"{f_res/1e6:.4f}", f"{D:.2f}", f"{efficiency:.2f}",
                     f"{Gc:.2f}", f"{Gr:.2f}", f"{gnd_offset:.2f}", f"{gnd_percent:.1f}"])
    ...
    added_title_val = f"Fres: {f_res/1e9:.2f} GHz, Eff: {efficiency:.2f}%, Gain: {Gc:.2f} dBi, Gr: {Gr:.2f} dBi"
...
if __name__ == "__main__":
    gnd_percents = np.concatenate((np.arange(0, 10, 2), np.arange(10, 20, 5), np.arange(20, 101, 20)))
    # gnd_percents = [100]
